# -*- coding: utf-8 -*-
"""
Created on Thu Jul 25 12:38:16 2024
...
folder = 'F:/20240723-Joaquin/20230818 - BeWo Tul/Fotos2'
...
field_identifiers = sorted([file.split("-") for file in files])
...
export = [["Tratamiento", "Campo", "PAR total", "PAR nuclear", "PAR citoplastamico", "Relacion"]]


for i in range(len(fields)):
    filepathDAPI = os.path.join(folder, fields[i][0])
    DAPI = mpimg.imread(filepathDAPI)
    DAPI = np.float_(DAPI)

    filepathPAR = os.path.join(folder, fields[i][1])
    ...
    row = [grouped[i][0][0], grouped[i][0][1], grouped[i][0][2], total, nuclear, citoplasmatico, nc]
