...
def montage(Nuc, Measure, Tub, cpMask, NucMask, back, field, masks):
    ...
    fig, ax = plt.subplots(nrows=3, ncols=3) #, figsize=(32, 64)
...
for i in range(len(fields)):
    ...
    for x in range(1, cellNum + 1):
        ...
        measure1 = background2nan(measure1, cell1)  #Isolates the fluorescence of interest from the chosen cell (the rest is set to nan)
...
with open('Field_results.csv', 'w', newline='') as f: #Measurements for each field's average value
    writer = csv.writer(f)
    writer.writerows(results)
    
with open('sc_results.csv', 'w', newline='') as f: #Individual measurements for detected cells in each field
    writer = csv.writer(f)
...
ax[1].imshow(Tub)
ax[1].axis('off')
ax[1].set_title("Tubuliuna")

ax[2].imshow(DAPI, cmap = "gray")
ax[2].imshow(Tub, alpha = 0.6, cmap = 'gray')
ax[2].axis('off')
ax[2].set_title('Merge')

ax[3].imshow(DAPI, cmap = "gray")
ax[3].imshow(Tub, alpha = 0.6, cmap = 'gray')
ax[3].imshow(masks[0], alpha = 0.4)
ax[3].axis('off')
